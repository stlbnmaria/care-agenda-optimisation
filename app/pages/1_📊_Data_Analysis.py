...
if (
    ...
    selected_caregivers = st.sidebar.multiselect(
        "Caregiver", caregiver_options, default="All"
    )
    ...
    selected_services = st.sidebar.multiselect(
        "Service", service_options, default="All"
    )
    ...
    filtered_df = jan24_df[
        ...
        & ((jan24_df["Date"] >= start_date) & (jan24_df["Date"] <= end_date))
    ]
    ...
    vehicule_personnel_percentage = (
        intervenants_df["Véhicule personnel"].value_counts(normalize=True)
        * 100
    ).get("Oui", 0)
    vehicule_personnel_percentage_formatted = (
        f"{vehicule_personnel_percentage:.2f}%"
    )
    permis_percentage = (
        intervenants_df["Permis"].value_counts(normalize=True) * 100
    ).get("Oui", 0)
    permis_percentage_formatted = f"{permis_percentage:.2f}%"
    competences_list = [
        comp.split(", ") for comp in intervenants_df["Compétences"]
    ]
    flatten_competences = [
        item for sublist in competences_list for item in sublist
    ]
    most_common_competence = str.lower(
        pd.Series(flatten_competences).value_counts().idxmax()
    )
    ...
    clients_per_service = filtered_df.groupby("Prestation")[
        "ID Client"
    ].nunique()
    ...
    wedges, _ = ax.pie(
        clients_per_service,
        labels=clients_per_service.index,
        startangle=140,
    ...
    center_circle = plt.Circle(
        (0, 0), 0.6, color="white", fc="white", linewidth=1.25
    )
    ...
    legend_labels = [
        ...
        for label, percent in zip(
            clients_per_service.index,
            clients_per_service / clients_per_service.sum() * 100,
        )
    ]
    ax.legend(
        wedges,
    ...
    filtered_df["Day of Week"] = pd.Categorical(
        filtered_df["Day of Week"], categories=days_order, ordered=True
    )
    service_by_day = (
        filtered_df.groupby(["Prestation", "Day of Week"])
        .size()
